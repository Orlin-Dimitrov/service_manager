...
// Custom validator for Requester, checks if Requester is NOT used in a Request
public class RequesterNotInUseValidator implements ConstraintValidator<RequesterNotInUse, Integer> {
	...
	@Override
	public void initialize(RequesterNotInUse constraintAnnotation) {
	...
	public boolean isValid(Integer theId, ConstraintValidatorContext context) {				
		...
		if (theId > 0) {
			...
			// If Requester is deleted inUse will be false. Validation will stop at @RequesterNotDeleted
			boolean inUse = requesterService.requesterInUse(theId);
